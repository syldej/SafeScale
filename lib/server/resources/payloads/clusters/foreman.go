...
func (b *foreman) construct(task concurrency.Task, req Request) (err error) {
	...
	defer func() {
		...
		metaErr := b.cluster.Alter(task, func(clonable data.Clonable) error {
			// Cluster created and configured successfully
			return b.cluster.Properties(task).Alter(Property.StateV1, func(clonable interface{}) error {
	...
	err = b.cluster.UpdateMetadata(task, func() error {
		err := b.cluster.GetProperties(task).Alter(clusterproperty.DefaultsV2, func(v interface{}) error {
		...
		err = b.cluster.GetProperties(task).Alter(clusterproperty.StateV1, func(v interface{}) error {
...
// VPL: moved to controller.go
// // complementHostDefinition complements req with default values if needed
// func complementHostDefinition(req *protocol.HostDefinition, def protocol.HostDefinition) *protocol.HostDefinition {
// 	var finalDef protocol.HostDefinition
// 	if req == nil {
// 		finalDef = def
// 	} else {
// 		finalDef = *req
// 		finalDef.Sizing = &protocol.HostSizing{}
...
// configureNode ...
func (b *foreman) configureNode(task concurrency.Task, index uint, pbHost *protocol.Host) error {
	if index < 1 {
...
// configureMaster ...
func (b *foreman) configureMaster(task concurrency.Task, index uint, pbHost *protocol.Host) error {
	if b.makers.ConfigureNode != nil {
...
func (b *foreman) getSwarmJoinCommand(task concurrency.Task, selectedMaster *protocol.Host, worker bool) (string, error) {
	clientInstance := client.New()
...
func (b *foreman) leaveNodeFromSwarm(task concurrency.Task, pbHost *protocol.Host, selectedMasterID string) error {
	ctx, err := task.GetContext()
	if err != nil {
		return err
	}
...
func (b *foreman) installNodeRequirements(task concurrency.Task, nodeType clusternodetype.Enum, pbHost *protocol.Host, hostLabel string) (err error) {
	tracer := concurrency.NewTracer(task, "", true).WithStopwatch().GoingIn()
	defer tracer.OnExitTrace()()
	defer scerr.OnExitLogError(tracer.TraceMessage(""), &err)()
...
func (b *foreman) taskCreateMaster(t concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	if pbHost != nil {
		...
		mErr := b.cluster.UpdateMetadata(t, func() error {
			...
			return properties.Alter(Property.NodesV2, func(v interface{}) error {
...
func (b *foreman) taskCreateNode(t concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	if pbHost != nil {
		mErr := b.cluster.UpdateMetadata(t, func() error {
			...
			return b.cluster.GetProperties(t).Alter(Property.NodesV2, func(v interface{}) error {
				nodesV2 := v.(*clusterpropsv2.Nodes)
...
func (b *foreman) installReverseProxy(task concurrency.Task) (err error) {
	...
	disabled := false
	err = b.cluster.GetProperties(task).Inspect(Property.FeaturesV1, func(v interface{}) error {
	...
	if !disabled {
		...
		target, err := features.NewClusterTarget(task, b.cluster)
		if err != nil {
			return err
		}
		results, err := feat.Add(target, features.Variables{}, features.Settings{})
		if err != nil {
			return err
		}
...
func (b *foreman) installRemoteDesktop(task concurrency.Task) (err error) {
	...
	if !disabled {
		...
		results, err := feat.Add(target, features.Variables{
			"Username": "cladm",
...
func (b *foreman) installProxyCacheClient(task concurrency.Task, pbHost *protocol.Host, hostLabel string) (err error) {
	defer scerr.OnPanic(&err)()
	...
	lockErr = b.cluster.RUnlock(task)
	if err != nil {
		if lockErr != nil {
			return scerr.Wrap(err, lockErr)
		}
		return err
	}
	if lockErr {
	...
	if !disabled {
		feature, err := features.NewFeature(task, "proxycache-client")
		if err != nil {
			return err
		}
		target, err := features.NewHostTarget(pbHost)
		if err != nil {
			return err
		}
		results, err := feature.Add(target, features.Variables{}, features.Settings{})
		if err != nil {
			return err
		}
		if !results.Successful() {
...
func (b *foreman) installProxyCacheServer(task concurrency.Task, pbHost *protocol.Host, hostLabel string) (err error) {
	defer scerr.OnPanic(&err)()
	...
	defer func() {
		lockErr := b.cluster.RUnlock(task)
		if lockErr != nil {
			log.Errorf("failed to unlock cluster")
		}
	}()

	err = b.cluster.GetProperties(task).Inspect(Property.FeaturesV1).ThenUse(func(v interface{}) error {
		_, disabled = v.(*clusterpropsv1.Features).Disabled["proxycache"]
		return nil
	})
	if err != nil {
...
func (b *foreman) installDocker(task concurrency.Task, pbHost *protocol.Host, hostLabel string) (err error) {
	if pbHost == nil {
		return scerr.InvalidParameterError("pbHost", "cannot be nil")
	}
	if hostLabel == "" {
...
func (b *foreman) buildHostname(task concurrency.Task, core string, nodeType NodeType.Enum) (cluid string, err error) {
	...
	outerErr := b.cluster.GetProperties(task).Alter(Property.NodesV2, func(v interface{}) error {
		nodesV2 := v.(*clusterpropsv2.Nodes)
		switch nodeType {
		case NodeType.Node:
