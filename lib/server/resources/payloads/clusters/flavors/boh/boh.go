...
import (
	...
	"github.com/CS-SI/SafeScale/lib/server/resources/payloads/clusters/flavors/boh/enums/ErrorCode"
...
func getGlobalSystemRequirements(task concurrency.Task, f flavors.Foreman) (string, error) {
	...
	if anon == nil {
		...
		identity, err := f.Cluster().Identity(task)
		if err != nil {
			return "", err
		}
...
func getNodeInstallationScript(task concurrency.Task, f flavors.Foreman, nodeType clusternodetype.Enum) (string, data.Map) {
	data := data.Map{}
	script := ""
