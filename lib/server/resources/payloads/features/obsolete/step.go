...
func (is *step) taskRunOnHost(t concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	if is.OptionsFileContent != "" {
		err := srvutils.UploadStringToRemoteFile(is.OptionsFileContent, host, srvutils.TempFolder+"/options.json", "cladm", "safescale", "ug+rw-x,o-rwx")
		if err != nil {
	...
	err = srvutils.UploadStringToRemoteFile(command, host, filename, "", "", "")
	if err != nil {
		return stepResult{err: err}, nil
	}
