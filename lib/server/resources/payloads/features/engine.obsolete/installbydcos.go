...
func (i *dcosInstaller) Check(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	// FIXME: validate parameters
	worker, err := newWorker(f.(*feature), t, method.DCOS, action.Check, nil)
	if err != nil {
		return nil, err
	}
...
func (i *dcosInstaller) Add(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	// FIXME: validate parameters
	worker, err := newWorker(f.(*feature), t, method.DCOS, action.Add, nil)
	if err != nil {
		return nil, err
	}
	err = worker.CanProceed(s)
...
func (i *dcosInstaller) Remove(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	// FIXME: validate parameters
	worker, err := newWorker(f.(*feature), t, method.DCOS, action.Remove, nil)
	if err != nil {
		return nil, err
	}
	err = worker.CanProceed(s)
