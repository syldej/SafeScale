...
func NewKongController(svc iaas.Service, network resources.Network, addressPrimaryGateway bool) (*KongController, error) {
	...
	var rn *abstracts.Network
	err = network.Inspect(voidtask, func(clonable data.Clonable) error {
		var ok bool
	...
	objh, err := network.Gateway(voidtask, addressPrimaryGateway)
	if err != nil {
		return nil, err
	}
	...
	if anon, ok := kongProxyCheckedCache.Get(network.Name()); ok {
		...
		setErr := kongProxyCheckedCache.SetBy(network.Name(), func() (interface{}, error) {
			results, err := rp.Check(addressedGateway, data.Map{}, install.Settings{})
			if err != nil {
	...
	privateIP, err := addressedGateway.PrivateIP(voidtask)
	if err != nil {
		return nil, err
	}
	publicIP, err := addressedGateway.PublicIP(voidtask)
	if err != nil {
		return nil, err
	}
