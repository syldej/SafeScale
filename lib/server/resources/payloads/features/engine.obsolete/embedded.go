...
// AllEmbeddedFeatures returns a clone of all embedded features
func AllEmbeddedFeatures() map[string]*feature {
	m := map[string]*feature{}
	for k, v := range allEmbeddedMap {
		m[k] = v
	}
	return m
}
...
func dockerFeature() *feature {
	...
	if err != nil {
		// panic is the good way to react
		panic(err.Error())
	}
	return &feature{
		name:            filename,
		filename:        filename,
		displayFilename: filename + " [embedded]",
		embedded:        true,
		specs:           specs,
	}
}
