...
func (g *genericPackager) Check(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	...
	worker, err := newWorker(f.(*feature), t, g.method, action.Check, g.checkCommand)
	if err != nil {
		return nil, err
	}
...
func (g *genericPackager) Add(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	...
	worker, err := newWorker(f.(*feature), t, g.method, action.Add, g.addCommand)
	if err != nil {
		logrus.Println(err.Error())
		return nil, err
	}
...
func (g *genericPackager) Remove(f install.Feature, t install.Targetable, v data.Map, s install.Settings) (install.Results, error) {
	...
	worker, err := newWorker(f.(*feature), t, g.method, action.Remove, g.removeCommand)
	if err != nil {
		return nil, err
	}
	err = worker.CanProceed(s)
