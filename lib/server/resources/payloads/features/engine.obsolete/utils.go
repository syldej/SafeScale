...
func UploadFile(localpath string, host resources.Host, remotepath, owner, group, rights string) (err error) {
	if localpath == "" {
	...
	retryErr := retry.WhileUnsuccessful(
		func() error {
			retcode, _, _, err := host.Push(task, localpath, remotepath, temporal.GetExecutionTimeout())
			if err != nil {
				return err
			}
			if retcode != 0 {
				...
				if retcode == 1 {
					...
					_, _, _, err = host.Run(task, fmt.Sprintf("sudo rm -f %s", localpath), temporal.GetBigDelay(), temporal.GetExecutionTimeout())
					if err == nil {
	...
	retryErr = retry.WhileUnsuccessful(
		func() error {
			var retcode int
			retcode, _, _, err = host.Run(task, cmd, temporal.GetDefaultDelay(), temporal.GetExecutionTimeout())
			if err != nil {
				return err
			}
			if retcode != 0 {
...
// func UploadStringToRemoteFile(content string, host *resources.Host, filename string, owner, group, rights string) error {
// 	if content == "" {
...
func findConcernedHosts(list []string, f install.Feature) (string, error) {
...
// // determineContext ...
// func determineContext(t Targetable) (hT *HostTarget, cT *ClusterTarget, nT *NodeTarget) {
// 	hT = nil
// 	cT = nil
// 	nT = nil
...
// func gatewayFromHost(host resources.Host) resources.Host {
