...
func (is *step) Run(hosts []resources.Host, v data.Map, s install.Settings) (res results, err error) {
	res = results{}
	...
	if is.Serial || s.Serialize {
		...
		for _, h := range hosts {
			...
			res[h.Name()] = result.(StepResults)
			err = subtask.Reset()
			if err != nil {
				return nil, err
			}

			if !res[h.Name()].Successful() {
				if is.Worker.action == action.Check { // Checks can fail and it's ok
					tracer.Trace("%s(%s):step(%s)@%s finished in %s: not present: %s",
						...
						temporal.FormatDuration(time.Since(is.Worker.startTime)), res[h.Name()].ErrorMessages())
		...
		for k, s := range subtasks {
			...
			res[k] = result.(StepResults)

			if !res[k].Successful() {
				if is.Worker.action == action.Check { // Checks can fail and it's ok
					tracer.Trace(": %s(%s):step(%s)@%s finished in %s: not present: %s",
						is.Worker.action.String(), is.Worker.feature.Name(), is.Name, k,
						temporal.FormatDuration(time.Since(is.Worker.startTime)), res[k].ErrorMessages())
				} else { // other steps are expected to succeed
					tracer.Trace(": %s(%s):step(%s)@%s failed in %s: %s",
						is.Worker.action.String(), is.Worker.feature.Name(), is.Name, k,
						temporal.FormatDuration(time.Since(is.Worker.startTime)), res[k].ErrorMessages())
...
// func (is *step) runOnHost(host *resources.Host, v Variables) stepResult {
func (is *step) taskRunOnHost(t concurrency.Task, params concurrency.TaskParameters) (result concurrency.TaskResult, err error) {
	...
	if is.OptionsFileContent != "" {
		err := host.PushStringToFile(t, is.OptionsFileContent, srvutils.TempFolder+"/options.json", "cladm", "safescale", "ug+rw-x,o-rwx")
		if err != nil {
	...
	filename := fmt.Sprintf("%s/feature.%s.%s_%s.sh", srvutils.TempFolder, is.Worker.feature.Name(), strings.ToLower(is.action.String()), is.Name)
	err = host.PushStringToFile(t, command, filename, "", "", "")
	if err != nil {
		return stepResult{err: err}, nil
	}
	...
	retcode, outrun, _, err := host.Run(t, command, temporal.GetConnectionTimeout(), is.WallTime)
	if err != nil {
